...
if (galleryImages.length > 0 && galleryModal) {
  ...
  const closeElements = galleryModal.querySelectorAll('[data-modal-close]');

  const toggleGalleryModal = (open) => {
    galleryModal.classList.toggle('open', open);
    galleryModal.setAttribute('aria-hidden', open ? 'false' : 'true');
    document.body.style.overflow = open ? 'hidden' : '';
  };
  ...
  closeElements.forEach(el => {
    el.addEventListener('click', () => toggleGalleryModal(false));
  });
